...
function actualizarStatusNube(estado) {
  ...
  const estados = {
    ...
    online:   { icon: 'fa-check-circle',          msg: 'Sincronizado con la nube ✅',                             color: '#d4edda' },
    offline:  { icon: 'fa-wifi',                  msg: 'Sin conexión — guardando localmente',                     color: '#ffe082', tachado: true },
    permisos: { icon: 'fa-exclamation-triangle',  msg: 'Reglas Firestore vencidas — ve a Firebase Console',       color: '#f8d7da' },
    error:    { icon: 'fa-times-circle',          msg: 'Error de conexión Firebase — verifica las credenciales',   color: '#f8d7da' }
  };
  ...
  // Si las reglas vencieron, mostrar alerta visible una vez
  if (estado === 'permisos' && !window._permisosAlertaMostrada) {
    window._permisosAlertaMostrada = true;
    setTimeout(() => {
      showToast('⚠️ Reglas Firebase vencidas. Ve a Firebase Console → Firestore → Reglas y extiende la fecha.');
...
function activarListenersFirebase() {
  ...
  // Test de permisos Firestore: detecta reglas vencidas (modo prueba 30 días)
  const pingRef = db.collection('_ping').doc('test');
  pingRef.set({ ts: Date.now() })
    .then(() => {
      actualizarStatusNube('online');
      pingRef.delete().catch(() => {});
    })
    .catch(err => {
      if (err.code === 'permission-denied') actualizarStatusNube('permisos');
      else if (err.code === 'unavailable') actualizarStatusNube('offline');
      else actualizarStatusNube('error');
    });

  // includeMetadataChanges:true para detectar cuando datos pasan cache→servidor
  // hasPendingWrites:true = cambio local propio, lo ignoramos para no crear loops
