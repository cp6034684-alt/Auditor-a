...
async function migrarAFirebase() {
  if (!db) return alert("Firebase no está configurado en el archivo app.js");
...
// ==========================================
// 8. ASISTENTE VIRTUAL RAG (CORREGIDO PARA FIREBASE)
// ==========================================
...
async function enviarAlAsistenteIA() {
  ...
  if (!geminiApiKey) { 
    ...
    let recordatorioVacio = (fechaManual) ? `${fechaManual} ${horaManual || '09:00'}` : null;
  ...
  RESPONDE SÓLO CON ESTE JSON ESTRICTO, NADA DE TEXTO EXTRA NI SALUDOS:
  {
    "accion": "pregunta",
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Google rechazó la conexión (Error ${response.status}). Verifica tu API Key.`);
    ...
    if (iaResult.accion === "pregunta") {
      ...
      // Guardar nota físicamente o en la Nube
      let recordatorioStr = null;
      if (fAg) { 
        recordatorioStr = hAg ? `${fAg} ${hAg}` : `${fAg} 09:00`; 
      }
...
// Función Guardar Nota (Adaptada para Nube o Local)
...
// ==========================================
// 9. RENDERIZADO PRINCIPAL Y DEPENDENCIAS (MARCAS AUDITORIA)
// ==========================================
function renderizarApp() {
      ...
      <div class="item" style="flex-direction:column; align-items:flex-start;">
        ...
        <div style="width:100%; display:flex; justify-content:space-between; margin-top:5px; align-items:center;">
          ...
          <div style="display:flex; gap:5px;">
            ...
            <button class="btn-icon delete" onclick="eliminarDato('distribuidores', ${d.id})" style="padding:4px 8px; width:auto;"><i class="fas fa-trash"></i></button>
          </div>
        </div>
      </div>`;
  });
      ...
      <div class="item" style="flex-direction:column; align-items:flex-start;">
        ...
        <div style="width:100%; display:flex; justify-content:space-between; margin-top:5px; align-items:center;">
          ...
          <div style="display:flex; gap:5px;">
            ...
            <button class="btn-icon delete" onclick="eliminarDato('vendedores', ${v.id})" style="padding:4px 8px; width:auto;"><i class="fas fa-trash"></i></button>
          </div>
        </div>
      </div>`;
  });
  ...
  // Render Aspectos Constantes
  document.getElementById("eval-diaria-container").innerHTML = aspectosDiarios.map((aspecto, index) => `<div class="cuali-item" style="color:var(--primary);"><span>${aspecto}</span><input type="checkbox" id="diario-${index}"></div>`).join('');
  document.getElementById("eval-visita-container").innerHTML = aspectosVisita.map((aspecto, index) => `<div class="cuali-item"><span>${aspecto}</span><input type="checkbox" id="visita-${index}"></div>`).join('');
  document.getElementById("pop-container").innerHTML = materialesPOP.map((pop, index) => `<div class="cuali-item"><span>${pop}</span><input type="checkbox" id="pop-${index}"></div>`).join('');
  
  // Render Marcas en Configuración
  let htmlMarcasConfig = "";
  marcas.forEach((m) => {
    const targetId = m.id ? m.id : `'${m.nombre}'`;
    htmlMarcasConfig += `
      <div class="item" style="flex-direction:column; align-items:flex-start;">
        ...
        <div style="width:100%; display:flex; justify-content:space-between; margin-top:5px;">
          ...
          <button class="btn-icon delete" onclick="eliminarDato('marcas', ${targetId})" style="padding:2px 5px; width:auto;"><i class="fas fa-trash"></i></button>
        </div>
      </div>`;
  });
...
function actualizarDependenciasAudit() {
  ...
  // Filtro de marcas para auditoría
...
// ==========================================
// 10. MÓDULO CRUD GENERAL (FIREBASE / LOCAL)
// ==========================================
function addSupTemp() { 
  ...
  try {
    if (idEdit) {
      ...
      if (dist) {
        ...
        if (db) await db.collection("distribuidores").doc(idEdit.toString()).set(dist);
        else localStorage.setItem("distribuidores", JSON.stringify(distribuidores));
      }
      ...
      if (db) await db.collection("distribuidores").doc(obj.id.toString()).set(obj);
      else { distribuidores.push(obj); localStorage.setItem("distribuidores", JSON.stringify(distribuidores)); }
      showToast("Distribuidor Creado");
    }
    
    document.getElementById("config-dist-id").value = "";
    document.getElementById("config-dist-nombre").value = ""; document.getElementById("config-dist-ciudad").value = ""; 
    document.getElementById("config-dist-direccion").value = ""; document.getElementById("config-dist-vendedores").value = ""; 
    document.getElementById("config-dist-clientes").value = ""; document.getElementById("config-sup-nombre").value = ""; document.getElementById("config-sup-tel").value = ""; 
    
    supervisoresTemp = []; renderSupTemp(); renderizarApp(); 
    switchTab('view-directorio', 'Directorio CRM'); cambiarVistaDir('distribuidores');
...
async function agregarVendedor() { 
  ...
  try {
    if (idEdit) {
      ...
      if(db) await db.collection("vendedores").doc(idEdit.toString()).set(ven);
      else localStorage.setItem("vendedoresObj", JSON.stringify(vendedores));
      ...
      if(db) await db.collection("vendedores").doc(obj.id.toString()).set(obj);
      else { vendedores.push(obj); localStorage.setItem("vendedoresObj", JSON.stringify(vendedores)); }
...
async function agregarMarca() { 
  ...
  try {
    if (db) await db.collection("marcas").doc(obj.id.toString()).set(obj);
    else { marcas.push(obj); localStorage.setItem("marcas", JSON.stringify(marcas)); renderizarApp(); }
    document.getElementById("nuevaMarca").value = ""; showToast("Marca Agregada"); 
  } catch(e) { alert("Error guardando: " + e.message); }
}

async function eliminarDato(tipo, id_o_index) { 
  if(!confirm("¿Eliminar registro permanentemente?")) return; 
  
  try {
    if(db) {
      if(tipo === 'distribuidores') await db.collection("distribuidores").doc(id_o_index.toString()).delete();
      if(tipo === 'vendedores') await db.collection("vendedores").doc(id_o_index.toString()).delete();
      if(tipo === 'marcas') await db.collection("marcas").doc(id_o_index.toString()).delete();
      if(tipo === 'notas') await db.collection("notas").doc(id_o_index.toString()).delete();
    } else {
      if(tipo === 'distribuidores') { distribuidores = distribuidores.filter(d => d.id !== id_o_index); localStorage.setItem("distribuidores", JSON.stringify(distribuidores)); }
      if(tipo === 'vendedores') { vendedores = vendedores.filter(v => v.id !== id_o_index); localStorage.setItem("vendedoresObj", JSON.stringify(vendedores)); }
      // Marcas usaba el índice por accidente en local, lo arreglamos buscando por nombre si no tiene ID
      if(tipo === 'marcas') { 
        if(typeof id_o_index === 'number') marcas = marcas.filter(m => m.id !== id_o_index); 
        else marcas = marcas.filter(m => m.nombre !== id_o_index);
        localStorage.setItem("marcas", JSON.stringify(marcas)); 
      }
      if(tipo === 'notas') { notasGlobales = notasGlobales.filter(n => n.id !== id_o_index); localStorage.setItem("notasGlobales", JSON.stringify(notasGlobales)); }
...
function guardarGCal() { 
  ...
  if (input.includes('src="')) { 
    ...
    if (match) { try { const urlObj = new URL(match[1]); input = urlObj.searchParams.get("src") || input; } catch(e){} } 
...
function guardarGemini() { 
  geminiApiKey = document.getElementById("gemini-key").value.trim(); localStorage.setItem("geminiKey", geminiApiKey); showToast("Clave IA Guardada"); 
}
...
function renderizarDirectorio() {
  ...
  if (contactosFiltrados.length === 0) {
    ...
    contactosFiltrados.forEach(c => {
      ...
      const btnEliminar = (c.tipo === 'Vendedor' && c.id) ? `<i class="fas fa-trash" style="color:var(--danger); cursor:pointer; font-size:1.1rem;" onclick="eliminarDato('vendedores', ${c.id})"></i>` : "";
      ...
      <div class="contacto-card" style="border-left-color:#673AB7; padding:0; overflow:hidden;">
        ...
        <div class="sesion-header" style="background:white; color:#333; border-bottom:1px solid #eee; padding:15px; display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="display:flex; gap:10px; align-items:center;">
             ...
             <i class="fas fa-trash" style="color:var(--danger); font-size:1.1rem; cursor:pointer;" onclick="eliminarDato('distribuidores', ${d.id})"></i>
...
// ==========================================
// 12. GUARDAR VISITA AUDITORIA (GPS)
// ==========================================
...
function restaurarBoton() { 
  ...
  btnGuardar.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Guardar Registro a la Nube'; 
}
...
// ==========================================
// 15. ESTADÍSTICAS Y PANEL DE CONTROL BI
// ==========================================
