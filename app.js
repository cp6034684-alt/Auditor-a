...
// Credenciales embebidas — cualquier dispositivo sincroniza automáticamente
// sin necesidad de configuración manual.
// ==========================================

// Credenciales del proyecto Firebase (embebidas por defecto)
const FIREBASE_CONFIG_DEFAULT = {
  apiKey:            "AIzaSyC8GSm4JmCjLJ_5x8R-Dn7E7uoSFvx6Lcg",
  authDomain:        "auditor-a.firebaseapp.com",
  projectId:         "auditor-a",
  storageBucket:     "auditor-a.firebasestorage.app",
  messagingSenderId: "543972963968",
  appId:             "1:543972963968:web:2a5cab6cdc1bc377341865",
  measurementId:     "G-N1H3F7R3L9"
};

// Si el usuario guardó credenciales personalizadas en Ajustes, esas tienen prioridad.
// Si no, usar las credenciales por defecto embebidas.
const _fbCfgGuardada = (() => {
  try { return JSON.parse(localStorage.getItem('firebaseConfig') || 'null'); } catch(_) { return null; }
})();

const _fbCfgActiva = _fbCfgGuardada || FIREBASE_CONFIG_DEFAULT;
  ...
  try {
    ...
    if (firebase.apps.length > 0) {
      ...
      if (app.options.projectId !== config.projectId) {
        alert('Para cambiar de proyecto Firebase necesitas recargar la página (F5).');
        return false;
      }
    } else {
    ...
    // SIN enablePersistence: siempre datos directos del servidor, sin caché intermedia.
    // Esto garantiza que cada dispositivo recibe los datos reales en tiempo real.
    db = firebase.firestore(app);

    // Activar listeners inmediatamente
...
// Intentar inicializar con config activa (embebida o guardada por usuario)
if (_fbCfgActiva) {
  inicializarFirebase(_fbCfgActiva);
}
...
function actualizarStatusNube(estado) {
  ...
  const estados = {
    ...
    online:   { icon: 'fa-check-circle',         msg: '☁️ Sincronizado',                                       color: '#d4edda' },
    offline:  { icon: 'fa-wifi',                 msg: 'Sin conexión — datos locales',                           color: '#ffe082', tachado: true },
    permisos: { icon: 'fa-exclamation-triangle', msg: '⚠️ Reglas vencidas — ve a Ajustes',                    color: '#f8d7da' },
    error:    { icon: 'fa-times-circle',         msg: 'Error Firebase — ve a Ajustes',                          color: '#f8d7da' }
  };
  ...
  // Actualizar texto en el sub-elemento (nuevo HTML con botón de sync)
  const txtEl = document.getElementById('nube-status-txt') || statusEl;
  txtEl.innerHTML = `<i class="fas ${cfg.icon}"${tachado}></i> ${cfg.msg}`;
  statusEl.style.color = cfg.color;

  // Mostrar botón de sync solo cuando hay Firebase configurado
  const btnSync = document.getElementById('btn-sync-nube');
  if (btnSync) btnSync.style.display = db ? 'inline-block' : 'none';
...
function activarListenersFirebase() {
  ...
  window.addEventListener('online',  () => { actualizarStatusNube('online'); sincronizarDesdeFirebase(); });
  window.addEventListener('offline', () => actualizarStatusNube('offline'));

  // Test de permisos: leer un documento
  ...
  // ────────────────────────────────────────────────────────────
  // LISTENERS SIN FILTROS DE CACHÉ: Firestore es siempre la fuente
  // de verdad. Se aceptan TODOS los snapshots del servidor.
  // ────────────────────────────────────────────────────────────
...
// Sincronización manual forzada desde Firestore (botón o al reconectar)
async function sincronizarDesdeFirebase() {
  if (!db) return;
  try {
    actualizarStatusNube('online');
    const [distSnap, vendSnap, marcasDoc, visitasSnap, notasSnap] = await Promise.all([
      db.collection("distribuidores").get(),
      db.collection("vendedores").get(),
      db.collection("marcas").doc("globales").get(),
      db.collection("visitas").get(),
      db.collection("notas").get()
    ]);

    distribuidores = distSnap.docs.map(d => d.data());
    vendedores     = vendSnap.docs.map(d => d.data());
    if (marcasDoc.exists) marcas = marcasDoc.data().lista || [];
    visitas        = visitasSnap.docs.map(d => d.data());
    notasGlobales  = notasSnap.docs.map(d => d.data());
...
function cargarConfigFirebaseEnFormulario() {
  // Usar config activa (guardada en ajustes o la embebida por defecto)
  const cfg = _fbCfgGuardada || FIREBASE_CONFIG_DEFAULT;
  ...
  if (statusEl) {
    if (db) {
      const origen = _fbCfgGuardada ? 'configuración personalizada' : 'configuración por defecto';
      statusEl.innerHTML = `<i class="fas fa-check-circle" style="color:#28a745;"></i> Firebase conectado y sincronizando (${origen}).`;
    } else {
      statusEl.innerHTML = '<i class="fas fa-info-circle" style="color:#F57C00;"></i> Reintentando conexión...';
    }
  }
}
...
function toggleGrabacionCentral() {
  ...
  if (isRecordingGlobal) {
    ...
    return; // onend se encarga del resto
  }
  ...
  if (status) { status.style.display = "block"; status.innerHTML = '<i class="fas fa-circle" style="color:red; animation:pulse 1s infinite;"></i> Escuchando… (toca de nuevo para detener)'; }
  document.getElementById("panel-nota-nueva").style.display = "none";

  voiceRecognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
  voiceRecognition.lang = 'es-CO';   // Español Colombia — mejor para términos locales
  voiceRecognition.interimResults = true;   // Mostrar texto en tiempo real
  voiceRecognition.continuous = true;        // Mantener escuchando mientras habla
  voiceRecognition.maxAlternatives = 1;

  let transcripcion = '';
  let silencioTimer = null;

  // Mostrar texto en tiempo real mientras habla
  voiceRecognition.onresult = function(event) {
    ...
    transcripcion += final;
    area.value = transcripcion + (interim ? '…' + interim : '');

    // Auto-detener si hay 3 segundos de silencio después de hablar
  ...
  voiceRecognition.onerror = function(event) {
    ...
    if (event.error === 'not-allowed' || event.error === 'permission-denied') {
      msgError = 'Permiso de micrófono denegado. Habilítalo en la configuración del navegador.';
    } else if (event.error === 'network') {
      msgError = 'Error de red. Verifica tu conexión a internet.';
    } else if (event.error === 'no-speech') {
      // no-speech no es fatal — el onend manejará el reset
      return;
    } else {
      msgError = `Error de micrófono: ${event.error}`;
    }
    resetBotonGrabacion();
    if (msgError) showToast(msgError);
  };

  voiceRecognition.onend = function() {
    clearTimeout(silencioTimer);
    const textoFinal = transcripcion.trim() || area.value.replace(/….*$/, '').trim();
    resetBotonGrabacion();

    if (textoFinal.length > 2) {
      area.value = textoFinal;
      document.getElementById("panel-nota-nueva").style.display = "block";
      // Auto-procesar con IA si hay texto
      setTimeout(() => { enviarAlAsistenteIA(); }, 300);
    } else if (area.value.trim().length === 0) {
      document.getElementById("panel-nota-nueva").style.display = "none";
...
function resetBotonGrabacion() {
  const btn    = document.getElementById("btn-grabar-central");
  const status = document.getElementById("status-grabacion");
  if (btn)    { btn.classList.remove("pulsing"); btn.innerHTML = '<i class="fas fa-microphone"></i>'; }
  if (status) status.style.display = "none";
...
async function guardarNotaFisica(titulo, original, procesado, urgente, stringRecordatorio) {
  ...
  try {
    ...
    localStorage.setItem("notasGlobales", JSON.stringify(notasGlobales));
    if (db) {
      db.collection("notas").doc(nuevaNota.id.toString()).set(nuevaNota)
        .catch(e => console.warn('Error al guardar nota en Firebase:', e.message));
    }
...
async function ejecutarGuardado(distribuidor, vendedor, zona, tienda, notas, comentarioGeneral, lat, lng) {
  ...
  try {
    // Siempre actualizar el array local inmediatamente (no esperar al onSnapshot)
    visitas.push(nuevaVisita);
    localStorage.setItem("visitas", JSON.stringify(visitas));

    if (db) {
      // También escribir a Firestore (el onSnapshot lo propagará a otros dispositivos)
      db.collection("visitas").doc(nuevaVisita.id.toString()).set(nuevaVisita)
        .catch(e => console.warn('Error al guardar visita en Firebase:', e.message));
    }
...
// ==========================================
// HUB DE DISTRIBUIDORES (Stats + Tareas)
// ==========================================
let _tabDistrib = 'stats'; // 'stats' | 'tareas'
...
function inicializarHubDistrib() {
  ...
  if (distribuidores.length === 0) {
    sel.innerHTML = '<option value="">— Sin distribuidores registrados —</option>';
  } else {
    sel.innerHTML = distribuidores.map(d => `<option value="${d.nombre}">${d.nombre}</option>`).join('');
    // Mantener la selección anterior si sigue existiendo
    if (seleccionPrevia && distribuidores.find(d => d.nombre === seleccionPrevia)) {
...
function cambiarTabDistrib(tab) {
  _tabDistrib = tab;
  const panelStats  = document.getElementById('panel-distrib-stats');
  const panelTareas = document.getElementById('panel-distrib-tareas');
  const btnStats    = document.getElementById('tab-distrib-stats');
  const btnTareas   = document.getElementById('tab-distrib-tareas');

  if (panelStats)  panelStats.style.display  = tab === 'stats'  ? 'block' : 'none';
  if (panelTareas) panelTareas.style.display = tab === 'tareas' ? 'block' : 'none';
  if (btnStats)    btnStats.classList.toggle('active',  tab === 'stats');
  if (btnTareas)   btnTareas.classList.toggle('active', tab === 'tareas');
...
function cambiarPeriodoDistrib(periodo) {
  ...
  ['semana', 'mes', 'anio'].forEach(p => {
    const btn = document.getElementById(`tab-ds-${p}`);
...
function getRangoPeriodoDistrib() {
  const hoy = new Date();
  const fin = getHoy();
  let inicio;
  if (_periodoDistrib === 'semana') {
    ...
    inicio = `${fin.substring(0, 7)}-01`;
  } else { // anio
    inicio = `${fin.substring(0, 4)}-01-01`;
  }
  return { inicio, fin };
}

function renderizarStatsDistrib() {
  const cont = document.getElementById('distrib-stats-content');
  if (!cont) return;

  const nombre = getDistribSeleccionado();
  if (!nombre) {
    cont.innerHTML = '<div class="card" style="text-align:center; color:var(--text-muted); font-size:0.9rem;">Registra un distribuidor en <b>Ajustes</b> o en el <b>Directorio CRM</b> para ver sus estadísticas.</div>';
    return;
  }

  const { inicio, fin } = getRangoPeriodoDistrib();
  const datos = visitas.filter(v => v.distribuidor === nombre && v.fechaISO >= inicio && v.fechaISO <= fin);

  const etiquetas = { semana: 'Últimos 7 días', mes: 'Mes actual', anio: 'Año actual' };
  const tiendas = datos.length;
  const impG = datos.filter(v => v.comproPorGenomma).length;
  const impD = datos.filter(v => v.comproPorDuracell).length;

  // Acompañamientos únicos (día + vendedor)
  const acompSet = new Set(datos.map(v => `${v.fechaISO}|${v.vendedor}`));
  const acomp = acompSet.size;

  // Disponibilidad promedio de marcas
  let sumaPct = 0, conResultados = 0;
  datos.forEach(v => {
    if (v.resultados && v.resultados.length) {
      sumaPct += v.resultados.filter(r => r.disponible).length / v.resultados.length * 100;
      conResultados++;
    }
  });
  const dispProm = conResultados ? Math.round(sumaPct / conResultados) : 0;

  // Efectividad de tácticas en tienda
  let cumple = 0, totalAsp = 0;
  datos.forEach(v => (v.evaluacionVisita || []).forEach(a => { totalAsp++; if (a.cumple) cumple++; }));
  const efectividad = totalAsp ? Math.round(cumple / totalAsp * 100) : 0;

  // Tareas pendientes de este distribuidor
  const tareasPend = (tareasDistrib[nombre] || []).filter(t => !t.hecha).length;

  // Top vendedores del período
  const porVend = {};
  datos.forEach(v => {
    if (!porVend[v.vendedor]) porVend[v.vendedor] = { tiendas: 0, impG: 0, impD: 0 };
    porVend[v.vendedor].tiendas++;
    if (v.comproPorGenomma) porVend[v.vendedor].impG++;
    if (v.comproPorDuracell) porVend[v.vendedor].impD++;
  });
  const topVend = Object.entries(porVend).sort((a, b) => b[1].tiendas - a[1].tiendas).slice(0, 5);

  let html = `
    <p style="font-size:0.78rem; color:var(--text-muted); margin-bottom:10px; text-align:center;">
      <i class="fas fa-calendar-alt"></i> ${etiquetas[_periodoDistrib] || ''} (${inicio} → ${fin})
    </p>
    <div class="kpi-grid">
      <div class="kpi-card">
        <i class="fas fa-store kpi-icon text-primary"></i>
        <div class="kpi-num">${tiendas}</div>
        <div class="kpi-label">Tiendas visitadas</div>
      </div>
      <div class="kpi-card" style="border-left-color:var(--eval);">
        <i class="fas fa-user-check kpi-icon" style="color:var(--eval);"></i>
        <div class="kpi-num">${acomp}</div>
        <div class="kpi-label">Acompañamientos</div>
      </div>
      <div class="kpi-card">
        <i class="fas fa-pills kpi-icon text-primary"></i>
        <div class="kpi-num">${impG}</div>
        <div class="kpi-label">Impactos Genomma</div>
      </div>
      <div class="kpi-card" style="border-left-color:#1D6F42;">
        <i class="fas fa-battery-full kpi-icon" style="color:#1D6F42;"></i>
        <div class="kpi-num">${impD}</div>
        <div class="kpi-label">Impactos Duracell</div>
      </div>
      <div class="kpi-card" style="border-left-color:var(--success);">
        <i class="fas fa-boxes kpi-icon text-success"></i>
        <div class="kpi-num">${dispProm}%</div>
        <div class="kpi-label">Disponibilidad marcas</div>
      </div>
      <div class="kpi-card" style="border-left-color:#673AB7;">
        <i class="fas fa-bullseye kpi-icon" style="color:#673AB7;"></i>
        <div class="kpi-num">${efectividad}%</div>
  ...
  if (tareasPend > 0) {
    html += `
      <div class="card" style="border-left:4px solid #dc3545; padding:10px 14px; font-size:0.85rem;">
        <i class="fas fa-exclamation-circle" style="color:#dc3545;"></i>
        <b>${tareasPend}</b> tarea${tareasPend > 1 ? 's' : ''} pendiente${tareasPend > 1 ? 's' : ''} con este distribuidor.
        <a href="#" onclick="cambiarTabDistrib('tareas'); return false;" style="color:var(--primary); font-weight:bold;">Ver tareas →</a>
      </div>`;
  }
  ...
  if (topVend.length > 0) {
    html += `
      <div class="card">
        <h3 style="font-size:0.95rem;"><i class="fas fa-user-tie" style="color:var(--primary);"></i> Vendedores del período</h3>
        ${topVend.map(([vend, s]) => `
          <div class="ficha-stat" style="background:var(--manage-item-bg);">
            <span><b>${vend}</b></span>
            <span>${s.tiendas} tiendas · G:${s.impG} · D:${s.impD}</span>
          </div>`).join('')}
      </div>`;
  } else if (tiendas === 0) {
    html += `
      <div class="card" style="text-align:center; color:var(--text-muted); font-size:0.85rem;">
        <i class="fas fa-info-circle"></i> Sin visitas registradas para <b>${nombre}</b> en este período.
      </div>`;
  }
...
let _calTareasAno  = new Date().getFullYear();
let _calTareasMes  = new Date().getMonth(); // 0-11
let _calDiaSelec   = getHoy();

function cambiarMesCalTareas(delta) {
  _calTareasMes += delta;
  if (_calTareasMes < 0)  { _calTareasMes = 11; _calTareasAno--; }
  if (_calTareasMes > 11) { _calTareasMes = 0;  _calTareasAno++; }
  renderizarTareasDistrib();
}

function irHoyCalTareas() {
  const hoy = new Date();
  _calTareasAno  = hoy.getFullYear();
  _calTareasMes  = hoy.getMonth();
  _calDiaSelec   = getHoy();
  renderizarTareasDistrib();
}

// Legacy stubs (evitan errores si algo los llama)
function cambiarVistaListaTareas(v) { 
  const sel = document.getElementById('filtro-scope-tareas');
  if (sel) sel.value = v;
  renderizarTareasDistrib();
}
function cambiarEstadoTareas() { renderizarTareasDistrib(); }

function getTodasLasTareasFiltradas() {
  const scope = document.getElementById('filtro-scope-tareas')?.value || 'este';
  let lista = [];
  if (scope === 'todas') {
    Object.keys(tareasDistrib).forEach(d => {
      (tareasDistrib[d] || []).forEach(t => lista.push({ ...t, distribuidor: d }));
    });
  } else {
    const nombre = getDistribSeleccionado();
    (tareasDistrib[nombre] || []).forEach(t => lista.push({ ...t, distribuidor: nombre }));
    if (nombre !== '_GLOBAL_') {
      (tareasDistrib['_GLOBAL_'] || []).forEach(t => lista.push({ ...t, distribuidor: '_GLOBAL_' }));
    }
  }
  return lista;
}
...
function renderizarTareasDistrib() {
  const tituloEl = document.getElementById('cal-tareas-titulo');
  const gridEl   = document.getElementById('cal-tareas-grid');
  if (!tituloEl || !gridEl) return;

  const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
  tituloEl.textContent = `${meses[_calTareasMes]} ${_calTareasAno}`;

  const todasTareas = getTodasLasTareasFiltradas();
  const hoy = getHoy();
  ...
  const primerDia = new Date(_calTareasAno, _calTareasMes, 1);
  const diasEnMes = new Date(_calTareasAno, _calTareasMes + 1, 0).getDate();
  // Lunes=0 … Domingo=6
  let diaInicio = primerDia.getDay(); // 0=dom
  ...
  for (let i = 0; i < diaInicio; i++) {
    celdas += `<div style="min-height:44px;"></div>`;
  }

  for (let d = 1; d <= diasEnMes; d++) {
    const fechaStr = `${_calTareasAno}-${String(_calTareasMes+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    const esHoy    = fechaStr === hoy;
    const esSelec  = fechaStr === _calDiaSelec;
    const tareas   = tareasPorFecha[fechaStr] || [];
    const pendientes = tareas.filter(t => !t.hecha);
    const hechas   = tareas.filter(t => t.hecha);
    const vencidas = pendientes.filter(t => fechaStr < hoy);

    const bgCelda  = esSelec  ? 'var(--primary)'
                   : esHoy    ? 'rgba(0,102,204,0.12)'
                   : 'var(--card-bg)';
    const colorNum = esSelec ? 'white' : esHoy ? 'var(--primary)' : 'var(--text-main)';
    const borderColor = esSelec ? 'var(--primary)' : vencidas.length > 0 ? '#dc3545' : 'var(--input-border)';

    // Puntos de color por categoría (máx 3 visibles)
    const puntos = pendientes.slice(0, 3).map(t => {
      const c = catColors[t.categoria] || '#888';
      return `<span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:${c};margin:0 1px;"></span>`;
    }).join('');
    const extraBadge = pendientes.length > 3 ? `<span style="font-size:0.6rem;color:var(--text-muted);">+${pendientes.length-3}</span>` : '';

    celdas += `
      <div onclick="seleccionarDiaCal('${fechaStr}')" style="
        min-height:44px; padding:3px; border-radius:6px; cursor:pointer;
        background:${bgCelda}; border:1px solid ${borderColor};
        transition:0.15s; position:relative;">
        <div style="font-size:0.8rem; font-weight:${esHoy||esSelec?'bold':'normal'}; color:${colorNum}; line-height:1;">${d}</div>
        ${tareas.length > 0 ? `
          <div style="display:flex; flex-wrap:wrap; gap:1px; margin-top:2px;">${puntos}${extraBadge}</div>
          ${hechas.length > 0 && hechas.length === tareas.length ? '<div style="font-size:0.55rem; color:#28a745; font-weight:bold;">✓</div>' : ''}
        ` : ''}
      </div>`;
  }

  gridEl.innerHTML = celdas;

  // Renderizar detalle del día seleccionado
  renderizarDetalleDia(_calDiaSelec, tareasPorFecha[_calDiaSelec] || []);
}

function seleccionarDiaCal(fechaStr) {
  _calDiaSelec = fechaStr;
  renderizarTareasDistrib();
}

function renderizarDetalleDia(fechaStr, tareas) {
  const container = document.getElementById('cal-tareas-detalle');
  if (!container) return;
  ...
  const fechaBonita = `${parseInt(dia)} de ${nombresMeses[parseInt(mes)-1]} de ${ano}`;
  const esHoy   = fechaStr === hoy;
  ...
  // Botón agregar directamente en el día
  let html = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
      <div>
        <span style="font-weight:bold; color:var(--text-main); font-size:0.9rem;">${esHoy ? '📅 Hoy — ' : ''}${fechaBonita}</span>
        ${pendientes.length > 0 ? `<span style="margin-left:6px; background:#dc3545; color:white; padding:1px 7px; border-radius:10px; font-size:0.72rem; font-weight:bold;">${pendientes.length} pendiente${pendientes.length>1?'s':''}</span>` : ''}
        ${completadas.length > 0 ? `<span style="margin-left:4px; background:#28a745; color:white; padding:1px 7px; border-radius:10px; font-size:0.72rem;">${completadas.length} ✓</span>` : ''}
      </div>
      <button onclick="abrirModalTareaEnFecha('${fechaStr}')" style="background:var(--primary); border:none; color:white; padding:4px 10px; border-radius:8px; font-size:0.8rem; cursor:pointer;">
        <i class="fas fa-plus"></i> Agregar
      </button>
    </div>`;

  if (tareas.length === 0) {
    html += `<div style="text-align:center; color:var(--text-muted); font-size:0.85rem; padding:15px 0;">
      Sin tareas para este día. Toca <b>Agregar</b> para crear una.
    </div>`;
  } else {
    // Ordenar: pendientes primero, luego por hora
    [...pendientes, ...completadas].forEach(t => {
      const color = catColors[t.categoria] || '#888';
      const icon  = catIcons[t.categoria] || 'fa-check';
      const esVencida = !t.hecha && t.fecha < hoy;
      const distribNombre = t.distribuidor === '_GLOBAL_' ? '🌐 Global' : t.distribuidor;
      const gcalLink = generarEnlaceCalendario(
        `[${t.categoria}] ${t.descripcion} — ${distribNombre}`,
        `Tarea: ${t.descripcion}. Distribuidor: ${distribNombre}.`,
        t.fecha, t.hora || '09:00'
      );

      html += `
        <div style="display:flex; align-items:flex-start; gap:10px; padding:10px 12px;
          background:${t.hecha ? 'var(--manage-item-bg)' : 'var(--card-bg)'}; border-radius:10px; margin-bottom:6px;
          border-left:4px solid ${t.hecha ? '#ccc' : color};
          ${esVencida ? 'box-shadow:inset 0 0 0 1px #dc3545;' : 'box-shadow:0 1px 4px var(--shadow-card);'}
          opacity:${t.hecha ? '0.7' : '1'};">

          <input type="checkbox" ${t.hecha ? 'checked' : ''}
            onchange="toggleTareaDistrib('${t.distribuidor}', ${t.id})"
            style="margin-top:2px; width:20px; height:20px; accent-color:${color}; flex-shrink:0; cursor:pointer;">

          <div style="flex:1; min-width:0;">
            <div style="font-size:0.9rem; font-weight:bold;
              ${t.hecha ? 'text-decoration:line-through; color:var(--text-muted);' : 'color:var(--text-main);'}">
              ${t.descripcion}
            </div>
            <div style="display:flex; gap:5px; margin-top:4px; flex-wrap:wrap; align-items:center;">
              <span style="background:${color}; color:white; padding:1px 7px; border-radius:8px; font-size:0.68rem;">
                <i class="fas ${icon}"></i> ${t.categoria}
              </span>
              <span style="font-size:0.72rem; color:${esVencida ? '#dc3545' : 'var(--text-muted)'};">
                ${t.hora ? `🕐 ${t.hora}` : ''}
              </span>
              <span style="font-size:0.72rem; color:var(--text-muted);">
                ${distribNombre !== (getDistribSeleccionado() || '') ? `• ${distribNombre}` : ''}
              </span>
              ${gcalLink ? `<a href="${gcalLink}" target="_blank"
                style="font-size:0.68rem; background:#4285F4; color:white; padding:1px 7px; border-radius:8px; text-decoration:none;">
                <i class="fas fa-calendar-plus"></i>
              </a>` : ''}
            </div>
          </div>

          <button onclick="eliminarTareaDistrib('${t.distribuidor}', ${t.id})"
            style="background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:0.95rem; padding:0;">
...
function abrirModalTareaEnFecha(fechaStr) {
  abrirModalTarea();
  setTimeout(() => {
    const el = document.getElementById('tarea-fecha');
    if (el) el.value = fechaStr;
  }, 50);
}
...
function abrirModalTarea() {
  ...
  const sel = document.getElementById('tarea-distribuidor');
  if (sel) {
    let opts = `<option value="_GLOBAL_">🌐 Global / Personal (todas las rutas)</option>`;
    if (distribuidores.length > 0) {
      opts += '<option disabled>──────────────────</option>';
      distribuidores.forEach(d => {
        opts += `<option value="${d.nombre}">${d.nombre}</option>`;
      });
    }
    sel.innerHTML = opts;
    const actual = getDistribSeleccionado();
    if (actual && distribuidores.find(d => d.nombre === actual)) sel.value = actual;
  }

  document.getElementById('tarea-descripcion').value = '';
  document.getElementById('tarea-fecha').value = _calDiaSelec || getHoy();
...
function guardarTareaDistrib() {
  ...
  tareasDistrib[nombre].push({ id: Date.now(), distribuidor: nombre, descripcion: desc, categoria: cat, fecha, hora, hecha: false, fechaCompletada: null, creada: getHoy() });
  localStorage.setItem('tareasDistrib', JSON.stringify(tareasDistrib));

  document.getElementById('modal-tarea').style.display = 'none';

  // Navegar al mes de la tarea guardada
  ...
  renderizarTareasDistrib();
  showToast(`Tarea agregada para ${fecha}`);
}
