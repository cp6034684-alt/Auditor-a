...
function abrirModalTarea() {
  ...
  // Poblar select de distribuidores + opción global
  const sel = document.getElementById('tarea-distribuidor');
  if (sel) {
    let opts = `<option value="_GLOBAL_">🌐 Global / Personal (todas las rutas)</option>`;

    // Usar distribuidores cargados; si aún están vacíos, mostrar solo Global
    ...
    // Pre-seleccionar el distribuidor activo (si hay uno)
    const actual = getDistribSeleccionado();
    if (actual && distribuidores.find(d => d.nombre === actual)) {
      sel.value = actual;
    }
  }
...
function renderizarTareasDistrib() {
  ...
  // Las tareas _GLOBAL_ aparecen SIEMPRE en todas las vistas
  let todasLasTareas = [];
  if (_vistaListaTareas === 'todas') {
    ...
    // Siempre incluir globales (a menos que estemos viendo _GLOBAL_ directamente)
    if (nombre !== '_GLOBAL_') {
      (tareasDistrib['_GLOBAL_'] || []).forEach(t => {
        todasLasTareas.push({ ...t, distribuidor: '_GLOBAL_' });
      });
    }
  }
  ...
  Object.keys(grupos).forEach(distrib => {
    // Header de grupo: en "todas" siempre; en "este distribuidor" solo para globales
    const esGlobal = distrib === '_GLOBAL_';
    const labelGrupo = esGlobal ? '🌐 Global / Personal' : distrib;
    const iconoGrupo = esGlobal ? 'fa-globe' : 'fa-truck';
    const colorGrupo = esGlobal ? '#673AB7' : 'var(--primary)';

    if (_vistaListaTareas === 'todas' || (esGlobal && _vistaListaTareas !== 'todas')) {
      html += `<div style="font-size:0.75rem; font-weight:bold; color:${colorGrupo}; margin:12px 0 6px; padding-left:4px; display:flex; align-items:center; gap:5px;">
        <i class="fas ${iconoGrupo}"></i> ${labelGrupo}
      </div>`;
    }
