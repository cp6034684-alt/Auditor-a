...
function restoreFormState() {
  try {
    ...
    // Restaurar distribuidor (si el select ya tiene opciones)
    ...
    if (selDistri && state.distribuidor) {
      ...
      if (opt) { selDistri.value = state.distribuidor; }
    }
    ...
    // Marcar pendientes para restaurar después de que actualizarDependenciasAudit corra
    ...
    // Guardar vendedor a restaurar — se aplica en actualizarDependenciasAudit
...
Análisis en máximo 150 palabras, formato directo y profesional:
...
5. ACCIÓN INMEDIATA: una sola acción concreta que el vendedor debe hacer en este momento.`;
  ...
  try {
    const texto = await llamarGemini({ prompt, imageBase64: base64Data, maxRetries: 1 });
