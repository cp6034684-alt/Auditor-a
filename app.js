...
async function enviarAlAsistenteIA() {
  ...
  msg.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Procesando con IA…';
  ...
  // Detección anticipada de palabras clave de calendario
  ...
  const refuerzoCalendario = quiereAgendar
    ? `\nATENCIÓN: El usuario mencionó palabras de calendario/agenda. Es MUY probable que quiera agendar. Prioriza "accion":"agendar" y deduce fecha y hora.`
    : '';

  const prompt = `Eres el Asistente IA de Cesar Pescador, Gestor Sell Out Genomma Lab.
FECHA HOY: ${fechaHoyTexto} (${diaSemana}).
CONTEXTO: ${formatearBaseDatosIA()}
Texto del usuario: "${truncar(textoOriginal, 800)}"
${refuerzoCalendario}

REGLAS ESTRICTAS:
1. Si es PREGUNTA sobre datos anteriores → "accion":"pregunta", "respuesta": tu respuesta completa
2. Si menciona calendario, agenda, evento, reunión, recordatorio, fecha futura, o quiere programar algo → "accion":"agendar"
   - "fecha_agendada": formato YYYY-MM-DD (calcula relativo a HOY ${fechaHoyTexto})
   - "hora_agendada": formato HH:MM (si no menciona hora, usa 09:00)
   - "titulo": título corto del evento (máx 6 palabras)
   - "resumen": descripción del evento
3. Para TODO lo demás → "accion":"nota"
   - "titulo": OBLIGATORIO, resume en 4-6 palabras (ej: "Faltante Suerox Tienda Norte")
   - "resumen": texto limpio y organizado

RESPONDE ÚNICAMENTE con JSON, sin texto adicional:
{"accion":"","respuesta":"","titulo":"","resumen":"","fecha_agendada":"","hora_agendada":""}`;

  try {
    const textoIA = await llamarGemini({ prompt, maxRetries: 2 });

    const jsonMatch = textoIA.match(/\{[\s\S]*?\}/);
    if (!jsonMatch) throw new Error("Respuesta IA sin formato JSON reconocible.");

    const iaResult = JSON.parse(jsonMatch[0]);

    if (iaResult.accion === "pregunta") {
      boxResp.innerHTML = `
        <div style="border-bottom:1px solid #ce93d8; padding-bottom:8px; margin-bottom:8px;">
          <b><i class="fas fa-robot"></i> Asistente Genomma Lab:</b>
        </div>
      ...
      boxResp.innerHTML = `
        <div style="border-bottom:1px solid #c3e6cb; padding-bottom:8px; margin-bottom:10px; color:#155724;">
          <b><i class="fas fa-calendar-check"></i> Evento detectado y guardado</b>
        </div>
        <div style="font-size:0.9rem; margin-bottom:10px;">
          <b>📌 ${titulo}</b><br>
          <span style="color:#666; font-size:0.82rem;">📅 ${fAg} a las ${hAg}</span><br>
          <span style="color:#555; font-size:0.82rem;">${resumen}</span>
        </div>
        <a href="${gcalLink}" target="_blank"
          style="display:block; background:#4285F4; color:white; padding:12px; border-radius:8px; text-align:center; text-decoration:none; font-weight:bold;">
      ...
      const titulo = (iaResult.titulo?.trim().length > 2)
        ? iaResult.titulo
        : textoOriginal.split(' ').slice(0, 5).join(' ') || "Nota de Campo";
      const resumen = iaResult.resumen || textoOriginal;
...
function mostrarRegistrosPorFecha() {
  ...
  Object.keys(grupos).forEach((clave, index) => {
    ...
    const gcalLink = generarEnlaceCalendario(gcalTitulo, gcalDetalle, fechaFiltro, horaInicio);
...
function exportarTodoElHistorial() {
  ...
  const encabezados = [
    ...
    "Compro_Genomma_Visita", "Compro_Duracell_Visita",
    "Latitud", "Longitud", "Link_Mapa",
    "Notas_Competencia", "IA_Visual_Exhibicion",
  ];
  nombresMarcas.forEach(m => encabezados.push(`Marca_${m.replace(/\s/g,'_')}`));
  materialesPOP.forEach(p => encabezados.push(`POP_${p.replace(/\s/g,'_')}`));
