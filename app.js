...
function inicializarFirebase(config) {
  ...
  try {
    // Reusar app si ya fue inicializada con el mismo proyecto
    let app;
    if (firebase.apps.length > 0) {
      app = firebase.apps[0];
      // Si el projectId cambió, no podemos reinicializar sin recargar
    ...
    // Habilitar persistencia offline (IndexedDB) para funcionar sin conexión
    firestoreInstance.enablePersistence({ synchronizeTabs: true })
      .catch(err => {
        if (err.code === 'failed-precondition') {
          console.warn('Persistencia offline: múltiples pestañas abiertas. Solo una sincroniza.');
...
async function llamarGemini({ prompt, imageBase64 = null, maxRetries = 2 }) {
  if (!geminiApiKey) {
...
// ==========================================
function actualizarStatusNube(conectado) {
  _fbConectado = conectado;
  const statusEl = document.getElementById('nube-status');
  if (!statusEl) return;
  if (!db) {
    statusEl.innerHTML = '<i class="fas fa-mobile-alt"></i> Solo local — configura Firebase en Ajustes';
    statusEl.style.color = '#ffe082';
    return;
  }
  if (conectado) {
    statusEl.innerHTML = '<i class="fas fa-check-circle"></i> Sincronizado con la nube';
    statusEl.style.color = '#d4edda';
  } else {
    statusEl.innerHTML = '<i class="fas fa-wifi" style="text-decoration:line-through;"></i> Sin conexión — guardando localmente';
    statusEl.style.color = '#ffe082';
  }
}
...
function activarListenersFirebase() {
  ...
  // Monitor de conectividad: usa eventos del navegador como fuente de verdad
  window.addEventListener('online',  () => actualizarStatusNube(true));
  window.addEventListener('offline', () => actualizarStatusNube(false));
  actualizarStatusNube(navigator.onLine);
  ...
  db.collection("distribuidores").onSnapshot(
    ...
    (querySnapshot) => {
      ...
      renderizarApp();
      if (!querySnapshot.metadata.fromCache) actualizarStatusNube(true);
    },
    (error) => {
      console.warn('Distribuidores sync error:', error.code);
      actualizarStatusNube(false);
    }
  );
...
// Activar listeners si Firebase está listo
if (db) {
  // Diferir levemente para no bloquear el render inicial
  setTimeout(activarListenersFirebase, 100);
} else {
  setTimeout(() => actualizarStatusNube(false), 200);
}
...
function guardarConfigFirebase() {
  ...
  if (ok) {
    showToast('Firebase configurado ✅ — sincronizando…');
    setTimeout(() => { activarListenersFirebase(); }, 500);
    if (statusEl) statusEl.innerHTML = '<i class="fas fa-check-circle" style="color:#28a745;"></i> Conectado. Los datos se sincronizan automáticamente.';
  } else {
...
function procesarFoto(event) {
  ...
  reader.onload = function(e) {
    const img = new Image(); 
    img.onload = function() {
      fotoBase64AI = comprimirImagen(img, 512, 0.55);   // Reducido para red móvil 
...
async function enviarAlAsistenteIA() {
  ...
  if (!geminiApiKey) { 
    alert("⚠️ Sin API Key configurada. La nota se guardará manualmente.");
    const recVacio = fechaManual ? `${fechaManual} ${horaManual || '09:00'}` : null;
    return guardarNotaFisica("Nota de Campo", textoOriginal, textoOriginal, urgente, recVacio);
  }

  btn.disabled = true; 
  msg.style.display = "block"; 
  msg.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Analizando…';
  ...
  const prompt = `Eres el Asistente IA de Cesar Pescador, Gestor Sell Out Genomma Lab.
FECHA HOY: ${fechaHoyTexto}.
BASE DE DATOS: ${formatearBaseDatosIA()}
Texto: "${truncar(textoOriginal, 600)}"

INSTRUCCIONES:
1. PREGUNTA sobre BD → "accion":"pregunta"
2. AGENDAR/RECORDAR → "accion":"agendar", deduce fecha_agendada (YYYY-MM-DD) y hora_agendada (HH:MM)
3. NOTA/COMENTARIO → "accion":"nota", deduce titulo (máx 5 palabras) y resumen

RESPONDE SOLO JSON (sin texto extra):
{"accion":"","respuesta":"","titulo":"","resumen":"","fecha_agendada":"","hora_agendada":""}`;

  try {
    const textoIA = await llamarGemini({ prompt, maxRetries: 1 });

    const jsonMatch = textoIA.match(/\{[\s\S]*\}/);
    if (!jsonMatch) throw new Error("La IA no devolvió JSON válido. La nota se guardó manualmente.");
    ...
    if (iaResult.accion === "pregunta") {
      ...
      const tituloFinal   = (iaResult.titulo?.length > 2) ? iaResult.titulo : "Nota de Campo";
      const resumenFinal  = iaResult.resumen || textoOriginal;
      const htmlFormat    = textoAHtml(resumenFinal);
      let fAg = fechaManual, hAg = horaManual;
      ...
      const recordatorioStr = fAg ? `${fAg} ${hAg || '09:00'}` : null;
    ...
    boxResp.innerHTML = `<div style="background:#f8d7da; color:#721c24; padding:10px; border-radius:8px; font-size:0.85rem;">
      ...
      <small>La nota fue guardada manualmente.</small>
    </div>`;
    boxResp.style.display = "block";
  }
}
...
async function analizarFotoIA() {
  ...
  const duracellTxt = vendeDuracell
    ? "Este distribuidor SÍ vende Duracell, incluye análisis de su exhibición."
    : "Este distribuidor NO vende Duracell.";

  const prompt = `Eres Cesar Pescador, Gestor Sell Out de Genomma Lab. Analiza esta foto de exhibición.
Marcas del distribuidor: ${marcasContexto}. ${duracellTxt}
1. PRESENCIA GENOMMA: ¿qué marcas del distribuidor se ven? ¿están bien exhibidas?
2. ANÁLISIS POR MARCA: visibilidad de cada marca visible.
3. COMPETENCIA DETECTADA (solo si las ves): Advil, Dolex, Advil Gripa, Electrolit, Energizer, Gaviscon, Pantene, Mieltertos.
4. RECOMENDACIONES: 2 acciones concretas.
Responde en máximo 120 palabras.`;

  try {
    const texto = await llamarGemini({ prompt, imageBase64: base64Data });
    const textHtml = textoAHtml(texto);
    box.innerHTML = `<b><i class="fas fa-camera"></i> Evaluación Visual IA:</b><br><br>${textHtml}`;
...
async function generarAnalisisIA(nombreVendedor, tiendas, pctMarcas, pctDiaria, pctVisita, indexBoton, popLogros, impGenomma, valGenomma, impDuracell, valDuracell, vendeDuracell) {
  ...
  const popTxt = popLogros > 0 ? `Logró POP en ${popLogros} tiendas, felicítalo.` : "";
  const ventasTxt = `Genomma Lab: ${impGenomma || 0} impactos de compra, valor $${Number(valGenomma||0).toLocaleString('es-CO')}.`
    + (vendeDuracell ? ` Duracell: ${impDuracell || 0} impactos, valor $${Number(valDuracell||0).toLocaleString('es-CO')}.` : '');

  const prompt = `Actúa como Cesar Pescador, Gestor Sell Out Genomma Lab. Escribe un WhatsApp motivador de coaching a ${nombreVendedor}.
Datos del día: ${tiendas} tiendas auditadas. Marcas ${pctMarcas}%. Tácticas ${pctVisita}%. Presentación ${pctDiaria}%. Ventas: ${ventasTxt} ${popTxt}
Estructura: 1.Saludo personalizado 2.Análisis numérico 3.Guion Genomma Lab (5 pasos) 4.Plan 3 pasos para mañana 5.Firma.
Tono cercano y motivador. Máximo 200 palabras.`;
...
async function generarReporteGerencial() {
  ...
  const prompt = `Eres Cesar Pescador, Gestor Sell Out Genomma Lab. Escribe un reporte breve para el SUPERVISOR (no gerencia).

DATOS:
- Ruta: ${codigoRuta} | Clientes: ${numClientesRuta} | Tiendas auditadas: ${totalTiendas}
- Vendedores: ${vendedoresActivos} | Distribuidores: ${distribuidoresAudit}
- Impactos Genomma: ${totalImpGenomma} | Impactos Duracell: ${totalImpDuracell}
- Marcas baja presencia: ${marcaBajaPresencia}
- Agotados hoy: ${agotadosDia}
- Novedades: ${observaciones}
- Análisis visual: ${analisisFotos}

Estructura (máximo 180 palabras):
1. RESUMEN (2 líneas)
2. ALERTA PRINCIPAL (lo más urgente HOY)
3. OPORTUNIDADES (máximo 2)
4. OBSERVACIÓN VISUAL (competencia, exhibición)
5. ACCIÓN REQUERIDA AL SUPERVISOR (1 concreta)`;
